...
- [ ] Drive `mypy --strict` over `src/corshub` to zero errors, then add `uv run mypy src/corshub` to the Docker `validate` stage as a blocking gate. The check is wired into `make lint` but was never enforced in CI, so ~124 strict-mode errors accumulated (missing generic type args, `Returning Any`, `register` ABC signature mismatch, missing pyrtcm stubs, Sanic `Stream`/`HTTPResponse` typing). See the NOTE in `docker/Dockerfile`.
- [ ] Raise the coverage ratchet toward the 75 % target. `tool.coverage.report.fail_under` is currently locked at 58 (the level at which coverage was re-enabled in CI) to prevent regression; ratchet it up as tests are added. Largest uncovered areas: `bin/`, `http/` (middleware, sessions, security, websocket), `oidc/`, `jwt`.
- [ ] `sanic-testing` is pinned at 24.6 while Sanic is at 25.12. Verify API compatibility and update the pin.
- [ ] Sanic deprecation (v26.6): listeners must take only the `app` argument; several `before_server_start` listeners (app code and test fixtures) still accept the deprecated `loop` argument and emit `DeprecationWarning`. Update signatures before the Sanic bump.
- [ ] Dropped-frame counter: frames silently evicted by the bounded queue drop-oldest policy are not counted. Add a `ntrip_frames_dropped_total` counter (labels: `mountpoint`) incremented in `QueueTransportSubscriber.publish` on eviction and on the `QueueFull` fallback path. Expose in Grafana. (Companion metric `ntrip_rtcm_quality_samples_dropped_total` was added for the quality-parse queue; mirror that approach.)
- [ ] Signal-quality parsing scale-out (Option B): `_observe_rtcm_quality` now runs off the delivery path via `NTRIPCaster._quality_loop` (bounded queue + single async worker), so pyrtcm decode no longer adds rover-delivery latency. It still runs on the event loop, so it does not add CPU. If parse cost ever saturates a core (watch `ntrip_rtcm_quality_samples_dropped_total`), swap `NTRIPCaster._process_quality` for a `ProcessPoolExecutor` that returns frame summaries to apply to the metrics — the call site is the single seam.
...
- [ ] Enforce `RTCM_UDP_MAX_DATAGRAM`: drop+count outgoing datagrams that would exceed the cap (today RTCM frames stay under one MTU, so this is latent).
- [ ] Dynamic NEAREST handoff: `KeepAlive` already carries position; re-subscribe a NEAREST session to a nearer base as the rover moves (currently position is only recorded).
